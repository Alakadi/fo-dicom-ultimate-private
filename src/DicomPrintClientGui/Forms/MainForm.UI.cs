...
public partial class MainForm
{
    ...
    private TabPage BuildDashboardTab()
    {
        ...
        openBrowserBtn.Click += (_, _) => System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo("http://localhost:9000/") { UseShellExecute = true });
    ...
    private TabPage BuildGlobalSettingsTab()
    {
        ...
        {
            Text      = "Admin API يُسخَدَم داخلياً فقط — لا تغير المنفذ إلا لضرورة",
            Location  = new Point(10, y), AutoSize = true,
            ForeColor = Color.Gray, Font = new Font("Segoe UI", 8f)
        };
    ...
    private void ShowWebViewError(string message)
    {
        ...
        Invoke(() =>
        {
            ...
            {
                System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo($"http://localhost:{_settings.AdminApi.Port}/")
                {
