...
namespace DicomPrintServer.Services
{
    ...
    public class PrintMonitor
    {
        ...
        private readonly PrintRepository? _db;

        public PrintMonitor(ILogger<PrintMonitor> logger, PrintRepository? db = null)
        {
            _logger = logger;
            _db     = db;
        }
            ...
            string? outputPath  = null)
        {
            ...
            // M5-A: حفظ في SQLite
            _db?.RecordSuccess(jobId, aet, null, patientName,
                pageCount, (long)duration.TotalMilliseconds, outputPath);
            ...
            int pageCount = 0)
        {
            ...
            // M5-A: حفظ في SQLite
            _db?.RecordFailure(jobId, aet, errorMessage, pageCount);
